...
class InputControls:
    ...
    Any options defined as 'integer' correspond to intergers in Fortran and must also be defined as integers here.
    '''

    def __init__(self, options=None):
        self.npoints = Control('npoints', 'Number of radial points', 51)
        # Switches for component models (Real)
        self.cmodel_weiland = Control('cmodel_weiland', 'Weiland', 1.0)
        self.cmodel_dribm = Control('cmodel_dribm', 'DRIBM', 1.0)
        self.cmodel_etg = Control('cmodel_etg', 'ETG', 1.0)
        self.cmodel_etgm = Control('cmodel_etgm', 'ETGM', 1.0)
        self.cmodel_mtm = Control('cmodel_mtm', 'MTM', 1.0)
        # Weiland options (Real)
        self.exbs_weiland = Control('exbs_weiland', 'ExB shear coefficient', 1.0)
        self.mpsf_weiland = Control('mpsf_weiland', 'Momentum pinch scaling factor', 1.0)
        self.lbetd_weiland = Control('lbetd_weiland', 'Lower bound of electron thermal diffusivity', 0.0)
        self.ubetd_weiland = Control('ubetd_weiland', 'Upper bound of electron thermal diffusivity', 100.0)
        self.lbitd_weiland = Control('lbitd_weiland', 'Lower bound of ion thermal diffusivity', 0.0)
        self.ubitd_weiland = Control('ubitd_weiland', 'Upper bound of ion thermal diffusivity', 100.0)
        # DRIBM options (Real)
        self.exbs_dribm = Control('exbs_dribm', 'ExB shear coefficient', 1.0)
        self.kyrhos_dribm = Control('kyrhos_dribm', 'kyrhos', 0.1)
        # MTM options (Real)
        self.ky_kx_mtm = Control('ky_kx_mtm', 'ky/kx for MTM', 0.2)
        self.cf_mtm = Control('cf_mtm', 'calibration factor', 1.0)
        # ETG options (Integer)
        ...
        # ETGM options (Real)
        self.exbs_etgm = Control('exbs_etgm', 'ExB shear coefficient', 0.0)
    ...
    def get_values(self):
        keys = self.get_keys()
        return [getattr(self, o).value for o in keys]

    def get_key_value_pairs(self):
        '''Returns (list): All key-value pairs of input controls'''

        keys = self.get_keys()
        return [str(o) + ', ' + str(getattr(self, o).value).replace('\n', '') for o in keys]
    ...
    def save_controls(self, options, scan_factor=None):
        ...
        if scan_factor is None:
            ...
            file_name = f'{save_dir}\\{DataType.CONTROLS.name.capitalize()}.csv'
        else:
            ...
            file_name = f'{save_dir}\\{DataType.CONTROLS.name.capitalize()} {options.var_to_scan} = {scan_factor_str}.csv'

        control_data = self.get_key_value_pairs()
...
class Control:
    ...
    def get_value_str(self):
        return self.value if type(self.value) is int else f'{self.value}D0'
...
* There needs to be existing folders corresponding to the chosen runid and scan_num when saving controls
'''
if __name__ == '__main__':
    ...
    print(controls.get_key_value_pairs())
