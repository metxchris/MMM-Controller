...
from plotting.plot_variables import FigData, PlotDataCsv, main


_log = logging.getLogger(__name__)


def plot_profiles(profile_list, fig_data_list, saveall=True):
    for profiles, fig_data in zip(profile_list, fig_data_list):
        print(f'Plotting profiles for {fig_data.savename_append}')
        for p in profiles:
            fig_data.set(*p)
            main(fig_data, savefig=saveall, savedata=saveall)
...
def get_compared_profiles(ptype, r, scan_nums, legends, savename_append, title=' '):
    ...
    if ptype == 'max':
        ...
        fig_data_max.title_override = title
        fig_data_max.savename_append = savename_append
    else:
        var_names = ['xteETGM', 'xte2ETGM']
        fig_data_sum.title_override = title
        fig_data_sum.summed_modes = True
        fig_data_sum.savename_append = f'sum_{savename_append}'
...
if __name__ == '__main__':
    ...
    plot_profiles([pmax, psum], [fig_data_max, fig_data_sum], saveall=1)
    ...
    # plot_profiles([pmax, psum], [fig_data_max, fig_data_sum], saveall=0)
